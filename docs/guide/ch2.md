...
**输出要求（跑出来应该长这样）**：程序输入一份简历文件（默认 `sample_resume.txt`，也可传自己的），输出是**终端打印的五板块结构化报告**——中间的 JSON 只是技术手段，对使用者可见的最终产物是这份报告：
...
[保留的优点]
+ 有完整的 RAG 项目经历，切中岗位方向

[问题清单]
1. [内容] 负责公司内部系统的开发维护工作
   -> 建议：拆出系统数量与量化结果（如「维护 3 个系统，工单时长从 2 天降到 4 小时」）
2. [表达] 工作认真负责，学习能力强，经常主动加班
   -> 建议：删掉主观评价词，换成事实证据；「加班」是减分项
3. [关键词] 技能栏缺少岗位高频词
   -> 建议：见底部关键词清单

[STAR 改写示例]
原句：负责整个项目的开发，用了大模型和向量数据库，实现了问答功能
改写：独立完成企业知识库问答系统——选型 GLM + Chroma，实现文档切分(300/50)
     与 top-3 语义检索，10 条测试集评估平均 4.6/5，Docker 部署上线
理由：原句是职责堆砌、无结果；改写补齐了 STAR 的行动与量化结果

[建议补充的 JD 关键词] RAG、Function Calling、FastAPI、Docker、流式输出
```

**验收口径**：跑 `sample_resume.txt`，五个板块齐全、每条 issue 带维度和建议、能通过 Pydantic 校验——「能跑」不算完成，「跑出来长这样」才算。

**开发路径（也是本章知识的完整复览）**：

1. `v0`：prompt 约束版——体验脆弱性（跑 5 次，总有一次解析失败？）
2. `v1`：开 JSON mode，system 里写清结构——稳定了
3. `v2`：加 Pydantic 校验 + 失败自纠重试 + 友好打印（格式对照上面的示例输出）
...
- [ ] 简历优化器跑 `sample_resume.txt` 输出五板块终端报告（口径见第 6 节示例）、通过 Pydantic 校验；看一眼 `usage` 能说出这次调用大约花了多少 token
- [ ] 已 push 到 GitHub
